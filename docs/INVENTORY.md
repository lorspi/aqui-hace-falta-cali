...
- `Consulta` — las piezas de la barra de consulta, una sola para la Radar y el Directorio: `BotonFiltros` («Filtrar y ordenar», con el conteo de aplicados), `ChipAplicado`, `QuitarTodos`, `ZonaChips` (fila que se desplaza, con › cuando desborda en móvil), `CampoBuscar`.
...
- `Tarjeta` — la tarjeta de una publicación en la lista (etiquetas, quién, dónde con distancia, recursos, el resumen de coincidencias, acciones en el pie). No se selecciona al tocarla: solo hover (borde navy-line y sombra leve); la selección del pin la lleva el mapa.
- `Coincidencias` — las sugerencias del cruce (el «Radar Match» de producción; en la interfaz se llaman sugerencias, manual § vocabulario): `Puntaje`, `ListaCoincidencias` (quién, porcentaje, distancia, qué tiene en común, Solicitar / Ayudar y Ver en el mapa), `DialogoCoincidencias` (la lista en un `<dialog>`, desde la tarjeta), `FilaSugerencias` (la fila a lo ancho con el radar de la marca, «N sugerencias cerca» y chevron, en degradado coral → navy —necesidad y oferta que se encuentran— con un brillo que la recorre una vez; `brillo` lo apaga) y `ResumenCoincidencias` (la fila dentro de la tarjeta). Al publicar (`flujos/comunes.tsx` `Coincidencias`): barrido del radar mientras busca y, si la mejor sugerencia es fuerte (`SUGERENCIA_FUERTE`, ≥ 90 %), ping corto (`utils/sonido.ts`) y brillo; `motion-reduce` apaga movimiento y sonido.
- `HojaFiltros` — la hoja lateral de filtros (Filtrar / Ordenar; lugar con `SelectorCiudad`, recurso por categoría, estado, solo verificadas; en Ordenar, «Más cerca» con su radio en km; «Ver N resultados»).
- `Opcion` — una opción de filtro como chip con `input` real (checkbox o radio). La usan las dos hojas y `SelectorCiudad`.
