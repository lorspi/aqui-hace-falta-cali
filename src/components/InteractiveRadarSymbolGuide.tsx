import React, { useState } from 'react';

type RadarLayer = 'red' | 'blue' | 'yellow' | null;

interface ColorItem {
  key: 'red' | 'blue' | 'yellow';
  title: string;
  label: string;
  description: string;
  colorHex: string;
  activeBgClass: string;
  activeBorderClass: string;
  activeRingClass: string;
  activeTextClass: string;
  accentBarClass: string;
  dotBgClass: string;
}

const COLOR_ITEMS: ColorItem[] = [
  {
    key: 'red',
    title: 'Rojo',
    label: 'El centro y la emergencia',
    description:
      'Es el punto exacto donde algo está pasando y alguien necesita ayuda. Es lo primero que se detecta y lo primero que se ve.',
    colorHex: '#CE3B3B',
    activeBgClass: 'bg-red-50/90',
    activeBorderClass: 'border-red-300',
    activeRingClass: 'ring-2 ring-red-500/40',
    activeTextClass: 'text-red-700',
    accentBarClass: 'border-l-brand-red',
    dotBgClass: 'bg-brand-red',
  },
  {
    key: 'blue',
    title: 'Azul',
    label: 'El arco interior y el apoyo cercano',
    description:
      'El vecino, la cuadra, el voluntario que ya está ahí. Rodea la emergencia de primero porque es el que llega de primero.',
    colorHex: '#1B3A93',
    activeBgClass: 'bg-blue-50/90',
    activeBorderClass: 'border-blue-300',
    activeRingClass: 'ring-2 ring-blue-600/40',
    activeTextClass: 'text-blue-800',
    accentBarClass: 'border-l-brand-blue',
    dotBgClass: 'bg-brand-blue',
  },
  {
    key: 'yellow',
    title: 'Amarillo',
    label: 'El arco exterior y la solidaridad amplia',
    description:
      'El apoyo que abraza desde más lejos: las organizaciones, las donaciones, la ciudad entera. La chispa de luz alrededor de lo que está pasando.',
    colorHex: '#F2C33D',
    activeBgClass: 'bg-amber-50/90',
    activeBorderClass: 'border-amber-300',
    activeRingClass: 'ring-2 ring-amber-400/50',
    activeTextClass: 'text-amber-800',
    accentBarClass: 'border-l-brand-yellow',
    dotBgClass: 'bg-brand-yellow',
  },
];

export const InteractiveRadarSymbolGuide: React.FC = () => {
  const [activeLayer, setActiveLayer] = useState<RadarLayer>(null);

  const handleMouseEnter = (layer: RadarLayer) => {
    setActiveLayer(layer);
  };

  const handleMouseLeave = () => {
    setActiveLayer(null);
  };

  const handleClick = (layer: 'red' | 'blue' | 'yellow') => {
    setActiveLayer((prev) => (prev === layer ? null : layer));
  };

  return (
    <section className="relative z-10 py-8 sm:py-10 bg-white border-y border-slate-200/80">
      <style>{`
        @keyframes radarRippleSequence {
          0% {
            transform: scale(1);
            opacity: 0;
          }
          2% {
            opacity: 0.92;
          }
          9% {
            opacity: 0.75;
          }
          18% {
            opacity: 0.45;
          }
          26% {
            opacity: 0.12;
          }
          29%, 100% {
            opacity: 0;
          }
          100% {
            transform: scale(5.5);
            opacity: 0;
          }
        }

        @keyframes radarRippleSoloActive {
          0% {
            transform: scale(1);
            opacity: 0;
          }
          5% {
            opacity: 0.92;
          }
          25% {
            opacity: 0.75;
          }
          55% {
            opacity: 0.45;
          }
          80% {
            opacity: 0.12;
          }
          92%, 100% {
            transform: scale(4.5);
            opacity: 0;
          }
        }

        .guide-ripple-1 {
          opacity: 0;
          animation: radarRippleSequence 11s cubic-bezier(0.15, 0.7, 0.3, 1) infinite;
          will-change: transform, opacity;
        }
        .guide-ripple-2 {
          opacity: 0;
          animation: radarRippleSequence 11s cubic-bezier(0.15, 0.7, 0.3, 1) infinite 3.66s;
          will-change: transform, opacity;
        }
        .guide-ripple-3 {
          opacity: 0;
          animation: radarRippleSequence 11s cubic-bezier(0.15, 0.7, 0.3, 1) infinite 7.33s;
          will-change: transform, opacity;
        }
        .guide-ripple-solo {
          opacity: 0;
          animation: radarRippleSoloActive 3.66s cubic-bezier(0.15, 0.7, 0.3, 1) infinite;
          will-change: transform, opacity;
        }
      `}</style>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center">
          <div className="lg:col-span-5 flex items-center justify-center">
            <div className="relative flex items-center justify-center w-60 h-60 sm:w-72 sm:h-72 md:w-80 md:h-80 lg:w-85 lg:h-85">
              <svg
                viewBox="40 0 980 1024"
                className="w-full h-full drop-shadow-sm overflow-visible select-none"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g className="pointer-events-none">
                  <circle
                    cx="612.823"
                    cy="515.77"
                    r="80.658"
                    fill="none"
                    stroke="#CE3B3B"
                    strokeWidth="3.5"
                    className={
                      activeLayer === null
                        ? 'guide-ripple-1'
                        : activeLayer === 'red'
                        ? 'guide-ripple-solo'
                        : 'opacity-0'
                    }
                    style={{ transformOrigin: '612.823px 515.77px' }}
                  />
                  <circle
                    cx="612.823"
                    cy="515.77"
                    r="80.658"
                    fill="none"
                    stroke="#1B3A93"
                    strokeWidth="3"
                    className={
                      activeLayer === null
                        ? 'guide-ripple-2'
                        : activeLayer === 'blue'
                        ? 'guide-ripple-solo'
                        : 'opacity-0'
                    }
                    style={{ transformOrigin: '612.823px 515.77px' }}
                  />
                  <circle
                    cx="612.823"
                    cy="515.77"
                    r="80.658"
                    fill="none"
                    stroke="#F2C33D"
                    strokeWidth="2.5"
                    className={
                      activeLayer === null
                        ? 'guide-ripple-3'
                        : activeLayer === 'yellow'
                        ? 'guide-ripple-solo'
                        : 'opacity-0'
                    }
                    style={{ transformOrigin: '612.823px 515.77px' }}
                  />
                </g>

                <g clipPath="url(#clip0_guide_interactive_radar)">
                  <path
                    d="M885.638 788.252L844.479 790.031L815.418 801.301L787.306 811.621L759.55 820.518L731.794 827.517L704.038 832.499L676.401 835.583L649.001 836.651L621.838 835.583L595.031 832.499L568.817 827.517L543.314 820.518L518.524 811.621L494.682 800.945L472.027 788.371L450.439 774.254L430.155 758.596L411.177 741.395L393.859 722.889L377.965 703.079L363.849 682.201L351.395 660.255L340.838 637.479L332.06 613.992L325.181 590.029L320.199 565.474L317.234 540.681L316.285 515.77L317.234 490.859L320.199 466.066L325.181 441.511L332.06 417.549L340.838 394.061L351.395 371.285L363.849 349.339L377.965 328.461L393.859 308.651L411.177 290.145L430.155 272.945L450.439 257.286L472.027 243.17L494.682 230.596L518.524 219.92L543.314 211.023L568.817 204.024L595.031 199.042L621.838 195.957L649.001 194.89L676.401 195.957L704.038 199.042L731.794 204.024L759.55 211.023L787.306 219.92L815.418 230.24L844.479 241.509L885.638 243.289C886.816 244.107 888.251 244.471 889.677 244.313C891.102 244.155 892.422 243.485 893.392 242.428C894.362 241.372 894.916 239.999 894.951 238.565C894.987 237.131 894.502 235.733 893.586 234.629L886.35 191.924L863.339 162.742L836.057 137.357L805.81 115.055L773.072 96.075L738.318 80.1793L701.903 67.7236L664.302 58.5895L625.752 53.0141L586.728 50.8789L547.347 52.3024L508.204 57.166L469.417 65.707L431.342 77.5695L394.334 92.8722L358.749 111.378L324.825 132.967L292.799 157.641L262.908 184.925L235.626 214.937L210.836 247.203L189.01 281.486L170.269 317.666L154.73 355.389L142.394 394.417L133.617 434.393L128.279 474.845L126.5 515.77L128.279 556.696L133.617 597.147L142.394 637.124L154.73 676.151L170.269 713.874L189.01 750.055L210.836 784.337L235.626 816.603L262.908 846.615L292.799 873.899L324.825 898.573L358.749 920.163L394.334 938.668L431.342 953.971L469.417 965.834L508.204 974.375L547.347 979.238L586.728 980.662L625.752 978.526L664.302 972.951L701.903 963.817L738.318 951.361L773.072 935.465L805.81 916.485L836.057 894.184L863.339 868.798L886.35 839.617L893.586 796.911C894.502 795.808 894.987 794.409 894.951 792.975C894.916 791.541 894.362 790.169 893.392 789.112C892.422 788.055 891.102 787.386 889.677 787.228C888.251 787.069 886.816 787.433 885.638 788.252Z"
                    fill="#F2C33D"
                    className="cursor-pointer transition-all duration-300 pointer-events-auto"
                    style={{
                      opacity: activeLayer === null || activeLayer === 'yellow' ? 1 : 0.22,
                      filter:
                        activeLayer === 'yellow'
                          ? 'drop-shadow(0 0 16px rgba(242,195,61,0.85))'
                          : 'none',
                      transform: activeLayer === 'yellow' ? 'scale(1.02)' : 'scale(1)',
                      transformOrigin: '612.823px 515.77px',
                    }}
                    onMouseEnter={() => handleMouseEnter('yellow')}
                    onMouseLeave={handleMouseLeave}
                    onClick={() => handleClick('yellow')}
                  />
                  <path
                    d="M481.871 385.044L503.697 384.688L519.117 379.706L533.944 375.198L548.415 371.64L562.649 369.03L576.882 367.488L590.879 367.013L604.638 367.606L618.042 369.267L631.208 371.877L643.9 375.554L656.117 380.181L667.86 385.756L679.01 392.28L689.448 399.517L699.293 407.583L708.308 416.361L716.611 425.851L724.084 435.934L730.608 446.492L736.301 457.643L741.046 469.149L744.723 481.012L747.57 493.112L749.349 505.449L750.179 517.904L749.824 530.479L748.637 543.053L746.265 555.508L742.944 567.845L738.555 579.945L733.099 591.808L726.694 603.196L719.339 614.228L711.036 624.667L701.784 634.513L691.583 643.884L680.552 652.544L668.691 660.373L656.117 667.609L642.714 674.133L628.48 680.183L613.535 686.352L596.454 700.112C595.196 700.065 593.97 700.52 593.047 701.376C592.124 702.233 591.579 703.421 591.531 704.679C591.484 705.938 591.939 707.163 592.795 708.087C593.652 709.01 594.84 709.555 596.098 709.602L613.653 724.786L633.225 729.175L653.271 730.362L673.554 728.938L693.837 725.142L713.646 719.211L732.862 711.026L751.366 700.824L768.802 688.724L785.171 674.964L799.998 659.424L813.401 642.461L825.144 624.192L835.108 604.738L843.174 584.453L849.223 563.219L853.138 541.629L855.035 519.565L854.798 497.501L852.426 475.437L847.919 453.728L841.276 432.613L832.736 412.091L822.298 392.636L809.962 374.249L795.846 357.167L780.308 341.509L763.346 327.393L745.198 315.174L725.863 304.735L705.817 296.194L685.06 289.788L663.946 285.399L642.477 283.027L621.126 282.908L600.012 284.925L579.255 288.958L559.327 295.127L540.23 303.312L522.319 313.395L505.832 325.376L491.242 339.136L479.143 355.151L475.466 378.045C474.569 378.903 474.044 380.077 474.003 381.317C473.961 382.557 474.407 383.764 475.245 384.679C476.082 385.594 477.245 386.145 478.484 386.213C479.722 386.282 480.938 385.862 481.871 385.044Z"
                    fill="#1B3A93"
                    className="cursor-pointer transition-all duration-300 pointer-events-auto"
                    style={{
                      opacity: activeLayer === null || activeLayer === 'blue' ? 1 : 0.22,
                      filter:
                        activeLayer === 'blue'
                          ? 'drop-shadow(0 0 16px rgba(27,58,147,0.85))'
                          : 'none',
                      transform: activeLayer === 'blue' ? 'scale(1.03)' : 'scale(1)',
                      transformOrigin: '612.823px 515.77px',
                    }}
                    onMouseEnter={() => handleMouseEnter('blue')}
                    onMouseLeave={handleMouseLeave}
                    onClick={() => handleClick('blue')}
                  />
                  <path
                    d="M612.823 596.435C657.369 596.435 693.481 560.321 693.481 515.77C693.481 471.22 657.369 435.105 612.823 435.105C568.277 435.105 532.165 471.22 532.165 515.77C532.165 560.321 568.277 596.435 612.823 596.435Z"
                    fill="#CE3B3B"
                    className="cursor-pointer transition-all duration-300 pointer-events-auto"
                    style={{
                      opacity: activeLayer === null || activeLayer === 'red' ? 1 : 0.22,
                      filter:
                        activeLayer === 'red'
                          ? 'drop-shadow(0 0 16px rgba(206,59,59,0.9))'
                          : 'none',
                      transform: activeLayer === 'red' ? 'scale(1.06)' : 'scale(1)',
                      transformOrigin: '612.823px 515.77px',
                    }}
                    onMouseEnter={() => handleMouseEnter('red')}
                    onMouseLeave={handleMouseLeave}
                    onClick={() => handleClick('red')}
                  />
                </g>

                <defs>
                  <clipPath id="clip0_guide_interactive_radar">
                    <rect width="771" height="949" fill="white" transform="translate(126.5 41.2695)" />
                  </clipPath>
                </defs>
              </svg>
            </div>
          </div>

          <div className="lg:col-span-7 space-y-3.5">
            <div>
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-brand-text tracking-tight font-sans">
                ¿Por qué el símbolo y los tres colores?
              </h2>
              <p className="text-slate-600 text-xs sm:text-sm mt-1 sm:mt-1.5 leading-relaxed font-body">
                Dos arcos abiertos alrededor de un punto. Se lee como una señal que se propaga desde un
                punto detectado, y como un abrazo que rodea a quien está en el centro. Los tres colores
                provienen de la bandera de Colombia y cuentan cómo funciona la red:
              </p>
            </div>

            <div className="space-y-2.5 font-sans">
              {COLOR_ITEMS.map((item) => {
                const isSelected = activeLayer === item.key;
                const isDimmed = activeLayer !== null && !isSelected;

                return (
                  <div
                    key={item.key}
                    onMouseEnter={() => handleMouseEnter(item.key)}
                    onMouseLeave={handleMouseLeave}
                    onClick={() => handleClick(item.key)}
                    className={`p-3 sm:p-3.5 rounded-2xl border-l-4 ${item.accentBarClass} border transition-all duration-300 flex items-start gap-3 cursor-pointer select-none ${
                      isSelected
                        ? `${item.activeBgClass} ${item.activeBorderClass} ${item.activeRingClass} shadow-md translate-x-1`
                        : isDimmed
                        ? 'bg-brand-surface/60 border-slate-200/50 opacity-60'
                        : 'bg-brand-surface border-slate-200/80 hover:bg-slate-100/80 hover:border-slate-300 shadow-2xs'
                    }`}
                  >
                    <span
                      className={`w-3.5 h-3.5 rounded-full ${item.dotBgClass} shrink-0 mt-0.5 transition-transform duration-300 ${
                        isSelected ? 'scale-125 ring-4 ring-black/10' : ''
                      }`}
                    />

                    <div className="flex-1 min-w-0">
                      <h4
                        className={`text-xs sm:text-sm font-extrabold tracking-tight transition-colors duration-200 ${
                          isSelected ? item.activeTextClass : 'text-brand-text'
                        }`}
                      >
                        {item.title} — {item.label}
                      </h4>
                      <p className="text-[11px] sm:text-xs text-slate-600 mt-0.5 leading-relaxed font-body">
                        {item.description}
                      </p>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
