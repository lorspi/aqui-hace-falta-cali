 ...
 * verdad —rótulos en mayúscula pequeña, filas con línea suave, cifras a la derecha, acciones
  ...
  return (
    <table aria-label={etiqueta} className="w-full border-collapse text-rd-13 max-xl:block">
      <thead className="max-xl:hidden">
        <tr>
          {columnas.map((c) => (
            <th key={c.k} scope="col" className={`font-rd border-b border-rd-line px-2 pb-2 text-left text-rd-11-5 font-semibold tracking-wider whitespace-nowrap text-rd-ink-meta uppercase ${c.num ? 'text-right' : ''}`}>
          ...
          <tr key={clave(f)} className="group max-xl:grid max-xl:grid-cols-2 max-xl:items-start max-xl:gap-3 max-sm:gap-2.5 max-xl:rounded-rd-xl max-xl:border max-xl:border-rd-line max-xl:bg-rd-surface max-xl:p-4 max-sm:p-3.5 max-sm:shadow-2xs xl:hover:bg-rd-fondo">
            {columnas.map((c, i) => {
              ...
              const clases = [
                ...
                c.acc ? 'xl:text-right xl:whitespace-nowrap xl:space-x-1.5 max-xl:col-span-2 max-xl:mt-1 max-xl:flex max-xl:flex-wrap max-xl:items-center max-xl:gap-1.5 max-xl:border-t max-xl:border-rd-line-soft max-xl:pt-2' : '',
              ]
                .filter(Boolean)
                .join(' ');
              return (
